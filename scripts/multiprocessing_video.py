...
def simulate_frequencies(
    ...
    tasks = [(base_cfg, omega) for omega in omegas]

    # Use ProcessPoolExecutor to bypass the GIL and utilize all CPU cores
...
def animate_comparison_split(
    ...
    f_data = forward_signal[::t_skip, :, :]
    b_data = backward_signal[::t_skip, :, :]
    ...
    time_to_mirror = base_cfg.L / base_cfg.c0
    frame_mirror = int((time_to_mirror / base_cfg.t_max) * n_frames_prop)
...
def main():
    ...
    animate_sigma_grid(
        results_dict,
        base_cfg=base_cfg,
        save_filename=f"output/2comparison_sigma_grid_{base_cfg.r_m}_{base_cfg.r0}.mp4",
        fps=30,
        t_skip=5,
        x_skip=5,
    )
